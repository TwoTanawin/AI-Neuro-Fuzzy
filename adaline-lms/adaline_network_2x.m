...
function R = find_R(x,bias_vector)
    sum_xTx = zeros(2, 2);  % Initialize a 2x2 matrix of zeros
    x_transpose = [x; bias_vector];
    x_transpose_transpose = [x' bias_vector'];
...
function widrow_hoff = find_widrow_hoff(x, y, bias_vector)
    ...
    weight_history = zeros(2, num_iterations);

    for i = 1:num_iterations
        % Display the updated weight vector for each iteration
        disp("---------Iteration " + i + "----------");
        disp("Present : " + x(i) + ", " + y(i));
        a = round((weight(1) * x(i)) + weight(2),2);
        disp("a = "+a)
        e = round(y(i) - a,2);
        disp("e = "+e)
        weight = round(weight + (2 * learning_rate * e * x_transpose(:, i)),2);
